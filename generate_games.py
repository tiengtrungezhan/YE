    ...
    <script>
        ...
        function renderQuestion() {{
            ...
            if (q.type === 0) {{
                ...
                html = `<div class="question-text">Đọc to từ sau:</div><div style="font-size: 3.5rem; font-weight: bold; color: var(--primary-dark); margin-bottom: 0.5rem;">${{q.word.hanzi}}</div><div style="color: #666; margin-bottom: 2rem; font-size: 1.2rem;">${{q.word.pinyin}} - ${{q.word.meaning}}</div><button class="audio-btn" id="record-btn" onclick="startRecording('${{q.word.hanzi}}')">🎤</button><div id="record-result" style="margin-top: 1rem; font-weight: bold; font-size: 1.1rem; min-height: 1.5rem;"></div>`;
            }} else if (q.type === 5) {{
