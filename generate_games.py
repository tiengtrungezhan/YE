...
<body>
    ...
    <script>
        ...
        let currentQuestion = 0, score = 0, questions = [], totalQuestions = Math.min(10, vocabulary.length);
        function initGame() {{ questions = generateQuestions(); renderQuestion(); }}
        function generateQuestions() {{
            let qs = [], shuffled = [...vocabulary].sort(() => Math.random() - 0.5);
            for (let i = 0; i < totalQuestions; i++) {{
                const type = Math.floor(Math.random() * 4), word = shuffled[i % shuffled.length];
                let q = {{ type, word }};
                if (type !== 3) q.options = generateOptions(word, type);
                qs.push(q);
            }}
            return qs;
        }}
