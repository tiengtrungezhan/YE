    ...
    <script>
        ...
        function renderQuestion() {{
            ...
            if (q.type === 0) {{
                ...
                html = `<div class="question-text">Nhấn giữ Micro và đọc to:</div><div style="font-size: 3.5rem; font-weight: bold; color: var(--primary-dark); margin-bottom: 0.5rem;">${{q.word.hanzi}}</div><div style="color: #666; margin-bottom: 2rem; font-size: 1.2rem;">${{q.word.pinyin}} - ${{q.word.meaning}}</div><button class="audio-btn" id="record-btn" style="user-select: none; touch-action: none;" onmousedown="startHoldRecord('${{q.word.hanzi}}')" onmouseup="stopHoldRecord()" onmouseleave="stopHoldRecord()" ontouchstart="startHoldRecord('${{q.word.hanzi}}')" ontouchend="stopHoldRecord()" oncontextmenu="return false;">🎤</button><div id="record-result" style="margin-top: 1rem; font-weight: bold; font-size: 1.1rem; min-height: 1.5rem;"></div>`;
            }} else if (q.type === 5) {{
        ...
        function startHoldRecord(correct) {{
            ...
            activeRecognition.onerror = function(e) {{
                if (e.error === 'no-speech') {{
                    res.innerText = "Chưa nghe rõ. Bạn hãy nhấn giữ lại.";
                }} else {{
                    res.innerText = "Lỗi thu âm. Bỏ qua câu này.";
                    showNextButton();
                }}
